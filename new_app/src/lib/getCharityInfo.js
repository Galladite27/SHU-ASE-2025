export function getAllCharityInfo(userId) {
    ...
    const [clothesStock] = db.prepare("select count(all) as itemsInStock from Stock where location_id = ? and status <> 'IN WAREHOUSE'").all(locationID.Location_ID);
    ...
    const incomingDonations = db.prepare("select donations.donation_id as id, concat(F_Name , ' ' , L_Name) as donorName, description as items,donations.donation_description as description,donations.status as status from Donations inner join user on (donor_id = user_id) inner join clothing on (donations.donation_id = clothing.donation_id) inner join stock on (stock.stock_id = clothing.item_id) where location_id = ?").all(locationID.Location_ID)
    db.close()
